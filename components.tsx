...
import { PieChart, Pie, Cell, Tooltip, Legend, ResponsiveContainer, BarChart, Bar, XAxis, YAxis, CartesianGrid } from 'https://esm.sh/recharts@^2.12.7';
import { ChevronDown, PlusCircle, Trash2, Edit, Save, X, Menu, FileDown, Settings, Sparkles, Loader as LoaderIcon, Copy as CopyIcon, Check, Upload, Link2, LayoutDashboard, List, PencilRuler, FileText, Sheet, Sun, Moon, LogOut, Wand2, FilePlus2, ArrowLeft, MoreVertical, User as UserIcon, LucideProps, AlertTriangle, KeyRound, ImageIcon, Download } from 'https://esm.sh/lucide-react@^0.523.0';
...
export const CopyBuilderPage: React.FC<CopyBuilderPageProps> = ({ planData, onPlanUpdate }) => {
    ...
    if (activeChannels.length === 0) {
        return (
            <Card className="text-center py-16">
                <PencilRuler size={48} className="mx-auto text-gray-400 mb-4" />
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                <div>
                    <label htmlFor="channel-select" className="text-sm font-medium text-gray-700 dark:text-gray-300 mr-2">{t('Canal')}:</label>
                    <select
                        id="channel-select"
                    ...
                    >
                        {activeChannels.map(ch => <option key={ch} value={ch}>{ch}</option>)}
                    </select>
                </div>
                <div className="flex items-center gap-2">
                     <div className="relative">
                        <button onClick={() => setExportType(prev => prev ? null : 'csv')} className="flex items-center gap-2 px-4 py-2 text-sm font-medium border rounded-md shadow-sm text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700">
                            <FileDown size={16} /> {t('export')}
                        </button>
                        {exportType && (
                            <div className="absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 z-10">
                                <div className="py-1">
                                    <button onClick={() => handleExport('csv')} className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">{t('export_as_csv')}</button>
                                    <button onClick={() => handleExport('txt')} className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">{t('export_as_txt')}</button>
                                </div>
                            </div>
                        )}
                    </div>
                    <button onClick={handleAddGroup} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-md shadow-sm hover:bg-blue-700">
                        <PlusCircle size={20}/> {t('Novo Grupo de Criativos')}
                    </button>
                </div>
            </div>

            <div>
                <h2 className="text-2xl font-bold mb-4 text-gray-900 dark:text-gray-100">{t('Criativos para')} {selectedChannel}</h2>
                {(planData.creatives?.[selectedChannel] || []).length > 0 ? (
                    (planData.creatives?.[selectedChannel] || []).map(group => (
                        <CreativeGroup
                            key={group.id}
                            group={group}
                    ...
                    <Card className="text-center py-16">
                        <PencilRuler size={48} className="mx-auto text-gray-400 mb-4" />
                        <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300">{t('Nenhum grupo de criativos para {channel}', {channel: selectedChannel})}</h3>
...
export const UTMBuilderPage: React.FC<UTMBuilderPageProps> = ({ planData, onPlanUpdate }) => {
    const { t } = useLanguage();
    // This component's implementation was missing from the provided file.
    // This is a placeholder to resolve compilation errors.
    return (
        <Card>
            <h1 className="text-2xl font-bold">{t('utm_builder')}</h1>
            <p className="mt-4 text-gray-500">Implementation pending.</p>
        </Card>
    );
};
...
export const KeywordBuilderPage: React.FC<KeywordBuilderPageProps> = ({ planData, onPlanUpdate }) => {
    ...
    return (
        <Card>
            <h1 className="text-2xl font-bold">{t('keyword_builder')}</h1>
            <p className="mt-4 text-gray-500">Implementation pending.</p>
        </Card>
    );
};
...
export const CreativeBuilderPage: React.FC<CreativeBuilderPageProps> = ({ planData, onPlanUpdate }) => {
    ...
    return (
        <Card>
            <h1 className="text-2xl font-bold">{t('creative_builder')}</h1>
            <p className="mt-4 text-gray-500">Implementation pending.</p>
        </Card>
    );
};
